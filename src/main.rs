...
impl Ball {
    ...
    pub fn draw(&self) {
        draw_rectangle(self.rect.x, self.rect.y, self.rect.h, self.rect.w, DARKGRAY);
    }
}
...
async fn main() {
    let font= load_ttf_font("res/Heebo-VariableFont_wght.ttf") // link in the description
        .await
        .unwrap();
    ...
    loop {
            ...
            GameState::Game => {
                ...
                for ball in balls.iter_mut() {
                    ...
                    for blocks in blocks.iter_mut() {
                        if resolve_collision(&mut ball.rect, &mut ball.vel, &blocks.rect) {
                            blocks.lives -= 1;
                            if blocks.lives <= 0 {
                                score += 10;
                                if blocks.block_type == BlockType :: SpawnBallOnDeath {
                ...
                blocks.retain(|block| blocks.lives > 0);
                if blocks.is_empty() {
        ...
        match game_state {
            ...
            GameState::LevelCompleted => {
                draw_title_text(&format!("you win!!!!!!! {} is your final score", score), font);
            }
            GameState::Dead => {
                draw_title_text(&format!("you lost {} is your final score", score), font);
            }
        }
        next_frame().await
    }
